document.addEventListener("DOMContentLoaded", function () {
            ...
            data.forEach(poll => {
                ...
                li.innerHTML = `
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 24px;">
                        <div>
                            <div>
                                <a href="/poll/${poll._id}" onclick="navigate('/poll/${poll._id}');return false;" style="font-weight:bold; font-size:1.1em;">${poll.title}</a>
                            </div>
                            <div style="margin:2px 0 4px 0; color:#444;">${poll.description || ""}</div>
    ...
    async function showPollDetails(pollId) {
        ...
        addOptionButton.onclick = async function () {
            ...
            if (window.PollOptions && PollOptions.pollId) {
                ...
                const optionsRes = await window.api.getOptionsByPollId(PollOptions.pollId);
    ...
    closeBtn.onmouseover = function () { closeBtn.style.color = "#e74c3c"; };
    ...
    if (!modal.querySelector("button[close-btn]")) {
    ...
    saveBtn.onclick = async function () {
        ...
        if (editingId) {
            ...
            const data = await window.api.updatePoll(editingId, { title, description, author_email });
            if (data && data._id) {
                modal.style.display = "none";
                modal.dataset.editing = "";
                showPollList();
            ...
            const data = await window.api.createPoll({ title, description, options, author_email });
            if (data.success) {
                modal.style.display = "none";
                showPollList();
            } else {
