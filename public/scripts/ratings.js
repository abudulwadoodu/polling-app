const Ratings = {
  ...
  rebindTotalRatings: function (ratings) {
    ...
    for (const option in ratings) {
      ...
      const avgRating = ratings[option].avgRating || 0;
      const userEmails = Array.isArray(ratings[option].user_emails) ? ratings[option].user_emails : [];
      ...
      emailsHtml = `<div style="font-size:12px; color:#555; margin-top:4px;">${userEmails.join(", ")}</div>`;
      }

      listItem.innerHTML = `<span style="width:125px; padding:10px">${option}:</span> <span style="width:20px; padding:10px"><strong>${avgRating}</strong></span> <span style="width:100px; padding:5px">${starRating}</span> <span style="padding:5px">(${ratingCount})</span>${emailsHtml}`;
